...
- `/api/smarttuple` – aggregates every note and returns a per-order summary generated by Gemini.

### LLM Integration
1. `app/api/chat/route.ts` initializes `GoogleGenAI` with an API key from `GOOGLE_API_KEY`.
2. `app/api/smarttuple/route.ts` exposes a GET endpoint that feeds all notes to Gemini and asks for a summary grouped by work order ID.
